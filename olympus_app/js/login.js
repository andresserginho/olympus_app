...
function validateName(v) {                               // ← BUG RAÍZ, faltaba esta función
  if (!v || !String(v).trim()) return 'El nombre es obligatorio';
  if (String(v).trim().length < 2) return 'Mínimo 2 caracteres';
  return '';
}
...
function validateEmail(v) {
  ...
  if (!e.includes('@')) return 'Sin "@" no es un correo válido';
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(e)) return 'Correo inválido';
  return '';
}

function validateEmailGmailHotmail(v) {
  if (!v || !String(v).trim()) return 'El correo es obligatorio';
...
function validatePassword(v) {
  ...
  if (!/[!@#$%^&*()\-_=+\[\]{};':"\\|,.<>/?]/.test(p)) return 'Falta un carácter especial (!@#$...)';
  return '';
}
...
function passwordStrengthScore(pass) {
  ...
  return [p.length >= 8, /[A-Z]/.test(p), /[a-z]/.test(p), /[0-9]/.test(p),
    /[!@#$%^&*()\-_=+\[\]{};':"\\|,.<>/?]/.test(p)].filter(Boolean).length;
}
...
function updateStrengthUI(pass) {
  ...
  if (!pass) { segs.forEach(el => { el.style.background = 'rgba(255,255,255,0.1)'; }); label.textContent=''; return; }
  const score = passwordStrengthScore(pass);
  segs.forEach((el,i) => { el.style.background = i < score ? colors[score] : 'rgba(255,255,255,0.1)'; });
...
window.screens.welcome = {
  render() {
    return `<div class="screen-container fade-in" style="padding:56px 32px;justify-content:space-between;align-items:center;">
      ${backgroundHTML()}
      <div class="flex items-center gap-3" style="z-index:1;">${olympusLogoSVG(28)}<span style="letter-spacing:0.3em;font-size:14px;color:var(--cyan);">OLYMPUS</span></div>
      <div class="flex flex-col items-center text-center gap-5" style="z-index:1;">
        ...
        <div>
          <h1 class="fade-in" style="font-size:28px;font-weight:700;text-transform:uppercase;letter-spacing:.02em;margin-bottom:10px;">¡Bienvenidos a <span style="color:var(--cyan);">OLYMPUS</span>!</h1>
...
window.screens.login = {
  render() {
    return `<div class="auth-screen slide-in-right">
      ${backgroundHTML()}
      <button type="button" class="link-back" id="btn-back-welcome"><span style="font-size:18px;">←</span> Volver</button>
      <div class="auth-header-row">${olympusLogoSVG(32)}
        <div><h2 class="auth-title">Iniciar Sesión</h2><p class="auth-subtitle">Continúa tu camino al siguiente nivel</p></div>
      </div>
      ...
      <form id="login-form" class="flex flex-col gap-4 auth-form-max">
        <div class="input-group">
          ...
          <div class="input-wrapper">
            <input type="email" id="login-email" class="input-field auth-field" autocomplete="email" placeholder="tu@email.com">
          </div>
        ...
        <div class="input-group">
          ...
          <div class="input-wrapper">
            <input type="password" id="login-pass" class="input-field auth-field" autocomplete="current-password" placeholder="••••••••" style="padding-right:44px;">
            <button type="button" class="input-icon" id="login-pass-toggle" aria-label="Mostrar contraseña"><i data-lucide="eye-off"></i></button>
          </div>
        ...
        <button type="submit" class="btn btn-primary auth-submit">Iniciar Sesión <i data-lucide="chevron-right"></i></button>
      </form>
      <div class="divider-or"><span class="divider-line"></span><span class="divider-text">o continúa con</span><span class="divider-line"></span></div>
      ...
      <div class="test-accounts-box">
        ...
        <button type="button" class="test-account-row" data-email="demo@olympus.com" data-password="Demo1234"><span>demo@olympus.com</span><span class="test-account-pass">Demo1234</span></button>
        <button type="button" class="test-account-row" data-email="admin@olympus.com" data-password="Admin1234"><span>admin@olympus.com</span><span class="test-account-pass">Admin1234</span></button>
        <button type="button" class="test-account-row" data-email="coach@olympus.com" data-password="Coach765"><span>coach@olympus.com</span><span class="test-account-pass">Coach765</span></button>
      </div>
      <p class="auth-footer">¿No tienes cuenta? <button type="button" class="link-cyan-inline" id="login-go-register">Regístrate gratis</button></p>
    </div>`;
  },
  init() {
    ...
    document.getElementById('login-forgot').addEventListener('click', () => alert('Contacta soporte para recuperar contraseña (demo).'));
    ...
    passIn.addEventListener('input', () => {
      ...
      toggleErr(passIn, document.getElementById('login-pass-err'), ''); clear();
    });
    emailIn.addEventListener('input', () => { toggleErr(emailIn, document.getElementById('login-email-err'), ''); clear(); });
    ...
    document.getElementById('login-pass-toggle').addEventListener('click', () => {
      ...
      document.getElementById('login-pass-toggle').innerHTML = show ? '<i data-lucide="eye"></i>' : '<i data-lucide="eye-off"></i>';
      if (window.lucide) lucide.createIcons();
    });

    document.querySelectorAll('.social-row .social-btn-dark').forEach(btn =>
      btn.addEventListener('click', () => alert('No disponible en esta demo.'))
    );
    ...
    form.addEventListener('submit', e => {
      ...
      /* Test accounts bypass strict validation */
      const isTest = window.AppState.testUsers.some(u => u.email.toLowerCase() === email.toLowerCase());
      const ee = isTest ? validateEmail(email) : validateEmailGmailHotmail(email);
      const pe = isTest ? (!pass ? 'La contraseña es obligatoria' : '') : validatePassword(pass);
      ...
      if (user) {
        Object.assign(window.AppState.user, { name: user.name, email: user.email, role: user.role, emailVerified: true });
        window.AppState.fromRegisterFlow = false;
        /* Persist session so reload keeps login */
...
window.screens.register = {
  render() {
    return `<div class="auth-screen slide-in-right">
      ${backgroundHTML()}
      <button type="button" class="link-back" id="btn-back-reg"><span style="font-size:18px;">←</span> Volver</button>
      <div class="auth-header-row">${olympusLogoSVG(32)}
        <div><h2 class="auth-title">Crear Cuenta</h2><p class="auth-subtitle">Comienza tu transformación hoy</p></div>
      </div>
      <form id="register-form" class="flex flex-col gap-4 auth-form-max">
        <div class="input-group">
          <label class="input-label" for="reg-name">Nombre completo</label>
          <input type="text" id="reg-name" class="input-field auth-field" autocomplete="name" placeholder="Tu nombre">
          <p id="reg-name-err" class="input-error hidden" role="status"></p>
        </div>
        <div class="input-group">
          <label class="input-label" for="reg-email">Correo electrónico</label>
          <input type="email" id="reg-email" class="input-field auth-field" autocomplete="email" placeholder="tu@gmail.com o tu@hotmail.com">
          <p id="reg-email-err" class="input-error hidden" role="status"></p>
        </div>
        <div class="input-group">
          <label class="input-label" for="reg-pass">Contraseña</label>
          <div class="input-wrapper">
            <input type="password" id="reg-pass" class="input-field auth-field" autocomplete="new-password" placeholder="Mínimo 8 caracteres" style="padding-right:44px;">
            <button type="button" class="input-icon" id="reg-pass-toggle" aria-label="Mostrar contraseña"><i data-lucide="eye-off"></i></button>
          </div>
          ...
          <!-- Requisitos en tiempo real (rojo/verde) -->
        ...
        <div class="input-group">
          ...
          <div class="input-wrapper">
            <input type="password" id="reg-pass2" class="input-field auth-field" autocomplete="new-password" placeholder="Repite la contraseña" style="padding-right:44px;">
            <button type="button" class="input-icon" id="reg-pass2-toggle" aria-label="Mostrar contraseña"><i data-lucide="eye-off"></i></button>
          </div>
          <p id="reg-pass2-err" class="input-error hidden" role="status"></p>
        </div>
        <label class="terms-row">
          <input type="checkbox" id="reg-terms">
          <span>Acepto los <button type="button" class="link-cyan-inline terms-link" data-link="terms">Términos</button> y la <button type="button" class="link-cyan-inline terms-link" data-link="privacy">Privacidad</button></span>
        </label>
        <p id="reg-terms-err" class="input-error hidden" role="status" style="margin-top:-8px;"></p>
        <button type="submit" class="btn btn-primary auth-submit" id="reg-submit">Crear mi cuenta <i data-lucide="chevron-right"></i></button>
      </form>
      <div class="divider-or"><span class="divider-line"></span><span class="divider-text">o regístrate con</span><span class="divider-line"></span></div>
      <div class="social-row">
        <button type="button" class="btn-social social-btn-dark">${iconGoogleSVG()} Google</button>
        <button type="button" class="btn-social social-btn-dark">${iconXSVG()} X</button>
      </div>
      <p class="auth-footer">¿Ya tienes cuenta? <button type="button" class="link-cyan-inline" id="reg-go-login">Inicia sesión</button></p>
    </div>`;
  },
  init() {
    ...
    document.querySelectorAll('.terms-link').forEach(btn =>
      btn.addEventListener('click', () => alert(btn.dataset.link === 'privacy' ? 'Política de privacidad (demo).' : 'Términos de uso (demo).'))
    );

    /* Toggle visibility */
    const bindToggle = (btnId, inp) => document.getElementById(btnId).addEventListener('click', () => {
      const show = inp.type === 'password';
      inp.type = show ? 'text' : 'password';
      document.getElementById(btnId).innerHTML = show ? '<i data-lucide="eye"></i>' : '<i data-lucide="eye-off"></i>';
      if (window.lucide) lucide.createIcons();
    });
    ...
    nameIn.addEventListener('input',  () => toggleErr(nameIn,  document.getElementById('reg-name-err'),  ''));
    emailIn.addEventListener('input', () => toggleErr(emailIn, document.getElementById('reg-email-err'), ''));
    terms.addEventListener('change',  () => document.getElementById('reg-terms-err').classList.add('hidden'));
    ...
    form.addEventListener('submit', e => {
      ...
      const ne  = validateName(name);                        // ← ahora existe
      const ee  = validateEmailGmailHotmail(email);
      ...
      } else { termsErrEl.textContent = ''; termsErrEl.classList.add('hidden'); }
      ...
      if (allUsers.some(u => u.email.toLowerCase() === email.toLowerCase())) {
        toggleErr(emailIn, document.getElementById('reg-email-err'), 'Este correo ya está registrado, usa otro');
        return;
      }

      /* Guardar nuevo usuario en localStorage (persiste al recargar) */
...
window.screens['verify-email'] = {
  render() {
    ...
    return `<div class="auth-screen slide-in-right verify-email-screen">
      ${backgroundHTML()}
      <button type="button" class="link-back" id="verify-back"><span style="font-size:18px;">←</span> Volver</button>
      <div class="verify-email-header">
        <h2 class="auth-title verify-title">VERIFICA TU EMAIL</h2>
        <p class="verify-lead">Enviamos un código a <strong class="verify-email-strong">${email}</strong></p>
      </div>
      <div id="verify-otp-err" class="alert-error hidden" role="alert"></div>
      <div class="otp-row" role="group" aria-label="Código de 6 dígitos">
        <input type="text" inputmode="numeric" maxlength="3" class="otp-box auth-field" id="otp-a" autocomplete="one-time-code">
        <span class="otp-dash" aria-hidden="true">—</span>
        <input type="text" inputmode="numeric" maxlength="3" class="otp-box auth-field" id="otp-b" autocomplete="one-time-code">
      </div>
      <p class="verify-spam-hint">¿No lo ves? Revisa spam.</p>
  ...
  init() {
    ...
    const showE = msg => { errEl.textContent = msg || ''; errEl.classList.toggle('hidden', !msg); };

    otpA.addEventListener('input', () => { otpA.value = otpA.value.replace(/\D/g,'').slice(0,3); if(otpA.value.length>=3&&!otpB.value) otpB.focus(); showE(''); });
    otpB.addEventListener('input', () => { otpB.value = otpB.value.replace(/\D/g,'').slice(0,3); showE(''); });
    otpA.addEventListener('keydown', e => { if(e.key==='Backspace'&&!otpA.value) otpB.focus(); });
    ...
    document.querySelector('.verify-email-screen').addEventListener('paste', ev => {
      ...
      ev.preventDefault();
      otpA.value = digits.slice(0,3); otpB.value = digits.slice(3,6); showE('');
    });

    document.getElementById('verify-submit').addEventListener('click', () => {
      if (!otpCodeValid(full())) { showE('Introduce un código de 6 dígitos.'); return; }
      window.AppState.user.emailVerified = true;
      window.AppState.user.pendingVerificationEmail = '';
      navigateTo('email-verified');
    });
    document.getElementById('verify-resend').addEventListener('click', () => alert('Email reenviado (simulado).'));
...
window.screens['email-verified'] = {
  render() {
    return `<div class="auth-screen slide-in-right email-verified-screen" style="position:relative;">
      ${backgroundHTML()}
      <button type="button" class="link-back" id="verified-back" style="position:absolute;top:24px;left:24px;z-index:1;"><span style="font-size:18px;">←</span> Volver</button>
      <div class="email-verified-inner">
        <div class="verified-check-wrap"><i data-lucide="check" class="verified-check-icon"></i></div>
        ...
        <button type="button" class="btn btn-primary auth-submit" id="verified-cta">CONFIGURAR MI PERFIL <i data-lucide="chevron-right"></i></button>
      </div>
    </div>`;
  },
  init() {
