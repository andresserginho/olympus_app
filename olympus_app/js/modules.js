...
window.screens['privacy-policy'] = {
  render() {
    return `
    <div class="screen-scroll" style="position:relative;background:var(--background,#080808);">
      <!-- Header con volver -->
      <div style="padding:52px 20px 0;display:flex;align-items:center;gap:12px;border-bottom:1px solid rgba(255,255,255,.07);padding-bottom:14px;">
        <button type="button" id="pp-back"
          style="width:38px;height:38px;border-radius:50%;background:rgba(255,255,255,.07);border:1px solid rgba(255,255,255,.1);display:flex;align-items:center;justify-content:center;cursor:pointer;flex-shrink:0;">
          <i data-lucide="arrow-left" style="width:18px;height:18px;"></i>
        </button>
        <div>
          <h2 style="font-size:18px;font-weight:700;">Política de Privacidad</h2>
          <p style="font-size:11px;color:#888;">Última actualización: Mayo 2025</p>
        </div>
      </div>

      <!-- Contenido -->
      <div style="padding:20px 24px 60px;display:flex;flex-direction:column;gap:20px;">

        <!-- Intro -->
        <div style="background:rgba(0,245,255,.04);border:1px solid rgba(0,245,255,.15);border-radius:14px;padding:16px;">
          <p style="font-size:13px;color:rgba(255,255,255,.8);line-height:1.6;">
            Olympus es una aplicación de fitness desarrollada como proyecto académico/demo. Tu privacidad es importante para nosotros. Esta política explica cómo manejamos tu información.
          </p>
        </div>

        <!-- Sección 1 -->
        ${ppSection('1. ¿Qué datos recopilamos?', `
          <p>Recopilamos únicamente los datos que tú nos proporcionas:</p>
          <ul>
            <li><strong>Datos de cuenta:</strong> Nombre, dirección de correo electrónico</li>
            <li><strong>Datos de perfil (opcionales):</strong> Edad, altura, peso, objetivo fitness, género</li>
            <li><strong>Datos de entrenamiento:</strong> Rutinas completadas, progreso semanal</li>
            <li><strong>Preferencias:</strong> Modo oscuro/claro</li>
          </ul>
        `)}

        <!-- Sección 2 -->
        ${ppSection('2. ¿Cómo almacenamos tus datos?', `
          <p>Todos tus datos se almacenan <strong>localmente en tu dispositivo</strong> usando localStorage del navegador:</p>
          <ul>
            <li>No utilizamos servidores externos</li>
            <li>No se transmite información a terceros</li>
            <li>Los datos permanecen en tu dispositivo hasta que los elimines</li>
            <li>Al limpiar el caché del navegador, los datos se eliminan</li>
          </ul>
        `)}

        <!-- Sección 3 -->
        ${ppSection('3. Cookies', `
          <p>Esta aplicación <strong>no utiliza cookies</strong>.</p>
          <p>Usamos localStorage (almacenamiento local del navegador), que es diferente a las cookies. El localStorage no se transmite automáticamente al servidor y no puede ser rastreado entre sitios web.</p>
        `)}

        <!-- Sección 4 -->
        ${ppSection('4. Seguridad de contraseñas', `
          <p>Sobre las contraseñas de los usuarios:</p>
          <ul>
            <li>Las contraseñas de las cuentas demo son de conocimiento público (prueba)</li>
            <li>En un entorno de producción, las contraseñas deben cifrarse con bcrypt o similar</li>
            <li>Nunca compartimos contraseñas con terceros</li>
            <li>Te recomendamos no usar contraseñas reales en aplicaciones demo</li>
          </ul>
        `)}

        <!-- Sección 5 -->
        ${ppSection('5. Tus derechos', `
          <p>Tienes derecho a:</p>
          <ul>
            <li><strong>Acceder</strong> a tus datos (visibles en tu perfil)</li>
            <li><strong>Modificar</strong> tus datos en cualquier momento (Perfil → Editar)</li>
            <li><strong>Eliminar</strong> tus datos cerrando sesión y limpiando el caché del navegador</li>
          </ul>
        `)}

        <!-- Sección 6 -->
        ${ppSection('6. Datos de menores', `
          <p>Olympus no está dirigida a menores de <strong>13 años</strong>. Al registrarte, confirmas que tienes al menos 13 años. No recopilamos intencionalmente datos de menores de esa edad.</p>
        `)}

        <!-- Sección 7 -->
        ${ppSection('7. Cambios a esta política', `
          <p>Podemos actualizar esta política de privacidad periódicamente. Te notificaremos de cambios significativos a través de la aplicación. El uso continuado de la app después de cambios constituye aceptación.</p>
        `)}

        <!-- Sección 8 -->
        ${ppSection('8. Contacto', `
          <p>Si tienes preguntas sobre esta política de privacidad:</p>
          <ul>
            <li>Email: privacidad@olympus-app.com</li>
            <li>La aplicación es un proyecto educativo/demo</li>
          </ul>
        `)}

        <div style="text-align:center;padding:10px 0;color:rgba(255,255,255,.3);font-size:11px;">
          © 2025 Olympus App · Todos los derechos reservados
        </div>
      </div>
    </div>`;
  },
  init() {
    document.getElementById('pp-back')?.addEventListener('click', () => {
      ...
      const prev = window.AppState.currentScreen;
      navigateTo(prev === 'privacy-policy' ? 'register' : 'register');
    });
    if (window.lucide) lucide.createIcons();
  }
};
...
window.screens['terms'] = {
  render() {
    ...
    <div class="screen-scroll" style="position:relative;background:var(--background,#080808);">
      <div style="padding:52px 20px 0;display:flex;align-items:center;gap:12px;border-bottom:1px solid rgba(255,255,255,.07);padding-bottom:14px;">
        ...
        <div>
          <h2 style="font-size:18px;font-weight:700;">Términos de Uso</h2>
          <p style="font-size:11px;color:#888;">Última actualización: Mayo 2025</p>
        </div>
      </div>
      ...
      <div style="padding:20px 24px 60px;display:flex;flex-direction:column;gap:20px;">
        ...
        <div style="background:rgba(251,191,36,.06);border:1px solid rgba(251,191,36,.2);border-radius:14px;padding:16px;">
          <p style="font-size:13px;color:rgba(255,255,255,.8);line-height:1.6;">
            Al usar Olympus, aceptas los siguientes términos. Lee cuidadosamente antes de continuar.
          </p>
        </div>

        ${ppSection('1. Aceptación de términos', `
          <p>Al registrarte y usar Olympus, aceptas estar vinculado por estos Términos de Uso. Si no estás de acuerdo, no debes usar la aplicación.</p>
        `)}

        ${ppSection('2. Uso permitido', `
          <p>Puedes usar Olympus para:</p>
          <ul>
            <li>Registrar y seguir tus entrenamientos personales</li>
            <li>Explorar rutinas y planes de ejercicio</li>
            <li>Monitorear tu progreso de fitness</li>
            <li>Fines educativos y personales</li>
          </ul>
        `)}

        ${ppSection('3. Uso prohibido', `
          <p>Queda prohibido:</p>
          <ul>
            <li>Intentar acceder a cuentas de otros usuarios</li>
            <li>Usar la aplicación con fines comerciales no autorizados</li>
            <li>Reproducir o distribuir el contenido sin permiso</li>
            <li>Usar la aplicación para actividades ilegales</li>
          </ul>
        `)}

        ${ppSection('4. Aviso médico importante ⚠️', `
          <p style="color:#fbbf24;font-weight:600;">La información de esta app NO reemplaza el consejo médico profesional.</p>
          <p>Antes de iniciar cualquier programa de ejercicio:</p>
          <ul>
            <li>Consulta con un médico o profesional de salud</li>
            <li>Las rutinas son de carácter informativo</li>
            <li>No nos responsabilizamos por lesiones derivadas del uso</li>
            <li>Si sientes dolor, para el ejercicio inmediatamente</li>
          </ul>
        `)}

        ${ppSection('5. Cuenta de usuario', `
          <ul>
            <li>Eres responsable de mantener tu contraseña segura</li>
            <li>Notifícanos de cualquier uso no autorizado de tu cuenta</li>
            <li>Una cuenta por persona</li>
            <li>Debes tener al menos 13 años para registrarte</li>
          </ul>
        `)}

        ${ppSection('6. Propiedad intelectual', `
          <p>El diseño, código, iconos y contenido de Olympus están protegidos. Olympus es un proyecto educativo/demo. Las imágenes de ejercicios provienen de fuentes libres de derechos.</p>
        `)}

        ${ppSection('7. Limitación de responsabilidad', `
          <p>Olympus se provee "tal cual" sin garantías. No garantizamos:</p>
          <ul>
            <li>Resultados específicos de fitness</li>
            <li>Disponibilidad ininterrumpida del servicio</li>
            <li>Ausencia de errores en los planes de entrenamiento</li>
          </ul>
        `)}

        ${ppSection('8. Modificaciones', `
          <p>Nos reservamos el derecho de modificar estos términos en cualquier momento. Los cambios entran en vigor al ser publicados en la aplicación.</p>
        `)}

        ${ppSection('9. Contacto', `
          <p>Para consultas sobre estos términos: legal@olympus-app.com</p>
        `)}
  ...
  init() {
    document.getElementById('terms-back')?.addEventListener('click', () => navigateTo('register'));
    if (window.lucide) lucide.createIcons();
  }
};

/* Helper para secciones de privacidad/términos */
function ppSection(title, content) {
  return `
    <div style="border-radius:12px;border:1px solid rgba(255,255,255,.07);overflow:hidden;">
      <div style="padding:14px 16px;background:rgba(255,255,255,.04);border-bottom:1px solid rgba(255,255,255,.07);">
        <h3 style="font-size:14px;font-weight:700;color:var(--cyan,#00f5ff);">${title}</h3>
      </div>
      <div style="padding:14px 16px;font-size:13px;color:rgba(255,255,255,.75);line-height:1.7;">
        <style>
          #olympus-legal ul { margin: 8px 0 0 16px; display:flex; flex-direction:column; gap:5px; }
          #olympus-legal li { list-style: disc; }
          #olympus-legal p  { margin-bottom: 6px; }
        </style>
        <div id="olympus-legal">${content}</div>
      </div>
    </div>`;
}